...
class Memory {
  ...
  private async generateSummary(contents: string[]): Promise<string> {
    ...
    const { text } = await u.Ai.Text(this.agentType as any).invoke({
      system: `Anda adalah asisten kompresi memori. Silakan kompres beberapa konten memori berikut menjadi ringkasan singkat, tidak melebihi ${summaryMaxLength} karakter. Hanya output konten ringkasan, jangan tambahkan awalan atau penjelasan apapun.`,
  ...
  getTools() {
    return {
      deepRetrieve: tool({
        description: "Pencarian mendalam memori: gunakan alat ini saat Anda perlu mengingat informasi historis detail terkait kata kunci tertentu",
        inputSchema: jsonSchema<{ keyword: string }>(
          z
            .object({
              keyword: z.string().describe("Kata kunci yang akan dicari"),
            })
            .toJSONSchema(),
        ),
        execute: async ({ keyword }) => {
          const results = await this.deepRetrieve(keyword);
          if (results.length === 0) return { found: false, message: "Tidak ditemukan memori terkait" };
